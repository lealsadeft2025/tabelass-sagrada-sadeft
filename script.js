...
/* --- LOCK --- */
$("btnEntrar").addEventListener("click", tryLogin);
$("senha").addEventListener("keydown", e=>{ if(e.key==="Enter") tryLogin(); });
function tryLogin(){
  const v = $("senha").value.trim();
  if(v === LOCK_PW){
    lock.style.display="none"; app.style.display="block"; initApp();
  } else {
    errLock.innerText = "Senha incorreta"; setTimeout(()=>errLock.innerText="",1800);
  }
}
...
/* --- build grid --- */
function buildGrid(){
  const grid = $("gridDezenas"); grid.innerHTML=""; gridElems=[];
  for(let i=1;i<=25;i++){
    const d = document.createElement("div"); d.className="dez"; d.innerText = i.toString().padStart(2,"0"); d.dataset.num = i;
...
$("btnLimpar").addEventListener("click", ()=>{
  selected.clear(); fixed.clear(); gridElems.forEach(e=>e.classList.remove("selected","fixed")); refreshSelectedUI();
});

/* incluir fixa */
$("btnIncluir").addEventListener("click", ()=>{
  selected.forEach(n=> fixed.add(n)); selected.clear();
...
function ensureHistoricoTextarea(){
  const arr = loadHistorico(); $("historico").value = arr.slice(-20).map(a=>a.join(" ")).join("\n"); $("lenHist").innerText = Math.min(arr.length,20);
}
if(!localStorage.getItem(HIST_KEY)) saveHistorico([]);

/* stats */
function construirStats(hist){
  const stats = {}; for(let i=1;i<=25;i++) stats[i]={dz:i,freq:0,lastSeen:-1,impulso:0,runs:0};
  hist.forEach((conc,idx)=>{ for(let d=1;d<=25;d++) if(conc.includes(d)) stats[d].freq++; });
  for(let d=1;d<=25;d++){ for(let i=hist.length-1;i>=0;i--) if(hist[i].includes(d)){ stats[d].lastSeen = hist.length-1 - i; break; } }
  for(let d=1;d<=25;d++){
    const tl = hist.map(c=> c.includes(d)?1:0);
    let runs=0,cur=0,imp=0;
    for(let i=0;i<tl.length;i++){ const v=tl[i], prev=i>0?tl[i-1]:0; if(v===1 && prev===0) imp+=1; if(v===1 && prev===1) imp+=2; if(v===0 && prev===1) imp-=1; if(v===1) cur++; else { if(cur>0){ runs++; cur=0; } } }
    if(cur>0) runs++;
...
function rankByScore(stats, arr){
  ...
  const score = d => { const s = stats[d]; return s.impulso + 1.2*(s.freq/maxF) + 0.8*(s.runs/maxR) + 1.0*((maxL - s.lastSeen)/maxL); };
  return arr.slice().sort((a,b)=>score(b)-score(a));
}

/* gerar */
function gerarSADEFT({concursoAnterior, historicoArr, quantidade, tamanho}){
...
function renderResultado(res){
  ...
  const meta = document.createElement("div"); meta.style.marginBottom="8px"; meta.style.color="var(--celeste)";
  meta.innerHTML = `<b>R</b>: ${res.R.join(" ")} &nbsp; <b>DzFA</b>: ${res.dzfa.join(" ")}`; cont.appendChild(meta);
  const oficial = toNums($("resultadoOficial").value||""); let high=0;
  res.jogos.forEach(j=>{
    const box = document.createElement("div"); box.className="jogo-box";
    const left = document.createElement("div");
    left.innerHTML = `<div style="color:var(--celeste);font-weight:800">Jogo ${j.id}</div><div class="jogo-num">${j.numeros.map(n=>`<span class="badge">${n.toString().padStart(2,"0")}</span>`).join(" ")}</div>`;
    const right = document.createElement("div"); right.style.textAlign="right"; right.innerHTML = `<div style="color:#ffd700;font-weight:800">Score: ${j.score}</div>`;
    let hits=0;
    if(oficial.length){ hits = j.numeros.filter(n=>oficial.includes(n)).length; right.innerHTML += `<div style="margin-top:6px;color:${hits>=10?'#7efc7e':'#a9d3ff'}"><b>Acertos: ${hits}</b></div>`; if(hits>=10) high++; } else right.innerHTML += `<div style="margin-top:6px;color:var(--muted)">Acertos: —</div>`;
    box.appendChild(left); box.appendChild(right); cont.appendChild(box);
  });
  $("probabilidades").innerText = `Jogos: ${res.jogos.length} • High≥10: ${high}`;
}

/* gerar evento */
$("btnGerar").addEventListener("click", ()=>{
  const conc = toNums($("concursoAnterior").value||""); const histTxt = ($("historico").value||""); const hist = histTxt.split("\n").map(l=>toNums(l)).filter(l=>l.length>0);
  const qtd = parseInt($("quantidade").value,10); const tam = parseInt($("tamanho").value,10);
  const histStore = loadHistorico();
  const histToUse = hist.length ? hist : histStore.slice(-20);
  if(conc.length < 5){ alert("Informe o concurso anterior com ao menos 5 dezenas."); return; }
  const res = gerarSADEFT({concursoAnterior:conc,historicoArr:histToUse,quantidade:qtd,tamanho:tam});
  // auto push concursoAnterior
  const arrHist = loadHistorico();
  const maybe = conc.slice(0,15);
  if(maybe.length>=5){
    if(!(arrHist.length && arraysEqual(arrHist[arrHist.length-1], maybe))){ arrHist.push(maybe); saveHistorico(arrHist); ensureHistoricoTextarea(); }
  }
  renderResultado(res); updateTendencias(3);
});
...
/* historico local helpers */
function loadHistorico(){ try{ return JSON.parse(localStorage.getItem(HIST_KEY)||"[]"); }catch(e){ return []; } }
function saveHistorico(arr){ localStorage.setItem(HIST_KEY, JSON.stringify(arr.slice(-50))); }
...
function loadVoices(){
  ...
  femaleVoice = v.find(voice => (voice.lang && voice.lang.toLowerCase().includes('pt')) && (voice.name.toLowerCase().includes('female') || voice.name.toLowerCase().includes('brasil') || voice.name.toLowerCase().includes('zira') || voice.name.toLowerCase().includes('victoria'))) || v.find(voice => voice.lang && voice.lang.toLowerCase().includes('pt')) || v[0];
}
...
function addChat(who, text, cls){ const d = document.createElement("div"); d.className = "chat-line "+cls; d.innerHTML = `<b>${who}:</b> ${text}`; $("chatLog").appendChild(d); $("chatLog").scrollTop = $("chatLog").scrollHeight; }

function handleChatSend(){
  const msg = $("chatInput").value.trim(); if(!msg) return; addChat("Você", msg, "chat-me"); $("chatInput").value="";
  const reply = advancedReply(msg); addChat("SADEFT", reply, "chat-ai"); speak(reply);
}
...
function advancedReply(msg){
  ...
  if(lower.includes("orar")||lower.includes("oração")||lower.includes("rezar")){
...
/* init */
function initApp(){ buildGrid(); ensureHistoricoTextarea(); updateTendencias(3); }
